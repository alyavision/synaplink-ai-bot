...
def check_environment():
    ...
    if not env_file.exists():
        ...
        logger.error("📝 Создайте файл .env на основе env_example.txt")
        return False
    
    # Проверяем размер файла .env (должен быть больше 0)
    if env_file.stat().st_size == 0:
        logger.error("❌ Файл .env пустой!")
        logger.error("📝 Заполните файл .env необходимыми данными")
        return False
